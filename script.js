...
// =========================
        ...
        if (canvas) {
            ...
            const ambient = new THREE.AmbientLight(0xffffff, 8);  // high ambient = no dark sides
            scene.add(ambient);
            ...
            sunLight2.position.set(-5, 5, -5);  // light from behind too
            scene.add(sunLight2);

            const purpleLight = new THREE.PointLight(0xBC95FF, 5, 20);
            purpleLight.position.set(0, 3, 6);
            ...
            model.traverse((child) => {
                if (child.isMesh) {
                    // Remove traverse entirely - just restore original materials
